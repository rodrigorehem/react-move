...
const Contador = () =>{
  ...
  return <div>
    <button type="button" onClick={() => handleContador(-1)}>-</button>
    {contador}
    <button type="button" onClick={() => handleContador(1)}>+</button>
  </div>
}
  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        <ComponeteFuncionalComArrow name={"Rodrigo2"} idade={10}></ComponeteFuncionalComArrow>
      </header>
    </div>
  );
}
...
const ComponeteFuncionalComArrow = ({name, idade}) =>{
  return (<div>
    <h3>Exemplo de componente {name} e tem {idade}</h3>
  </div>)
}
